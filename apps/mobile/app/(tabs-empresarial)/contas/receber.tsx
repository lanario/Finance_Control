...
/**
 * Listagem de contas a receber (Fase 4).
 */
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabaseEmpresarial
        ...
        .select('id, descricao, valor, data_vencimento, recebida, parcelada')
        .eq('user_id', userId)
        .order('data_vencimento', { ascending: false })
        .limit(100)
      if (error) throw error
      setLista((data as ContaReceber[]) ?? [])
    } catch (e) {
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>Nenhuma conta a receber.</Text>}
        renderItem={({ item }) => (
          <View style={[styles.card, item.recebida && styles.cardRecebida]}>
            <Text style={styles.cardDesc}>{item.descricao}</Text>
            <Text style={styles.cardValor}>{formatarMoeda(item.valor)}</Text>
            <Text style={styles.cardData}>Venc.: {new Date(item.data_vencimento).toLocaleDateString('pt-BR')} {item.recebida ? '• Recebida' : ''}</Text>
          </View>
        )}
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  cardRecebida: { opacity: 0.7 },
  cardDesc: { fontSize: 16, color: '#fff', fontWeight: '600' },
  cardValor: { fontSize: 16, color: '#10b981', marginTop: 4 },
