...
export default function CategoriaNovaScreen() {
  ...
  async function handleSalvar() {
    ...
    setLoading(true)
    try {
      const { error } = await supabasePessoal.from('tipos_gastos').insert({
        user_id: userId,
        nome: nome.trim(),
      ...
      Alert.alert('Erro', 'Não foi possível salvar. Nome pode já existir.')
    } finally {
      setLoading(false)
    }
  }
