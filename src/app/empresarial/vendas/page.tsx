...
export default function VendasPage() {
  ...
  const loadVendas = async () => {
    try {
      ...
      // Processar dados - buscar nomes de clientes, categorias, produtos e serviços
  ...
  }, [filtroStatus, filtroCliente, filtroCategoria, filtroDataInicio, filtroDataFim, buscaTexto, abaAtiva, session])
      ...
      if (editingVenda) {
        ...
        if (statusAnterior !== novoStatus) {
          if (novoStatus === 'aprovado' && statusAnterior !== 'aprovado') {
            ...
            // Remover movimentação do fluxo de caixa
            await supabase
              .from('fluxo_caixa')
              .delete()
              .eq('user_id', userId)
        ...
        // Se status for aprovado, criar movimentação no fluxo de caixa
        ...
        if (formData.parcelada && novaVenda) {
          ...
          for (let i = 0; i < totalParcelas; i++) {
            ...
            // Se parcela for aprovada, criar movimentação no fluxo de caixa
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      // Deletar movimentações do fluxo de caixa relacionadas à venda principal
      await supabase
        .from('fluxo_caixa')
        .delete()
        .eq('user_id', userId)
  ...
  const handleCriarServico = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          user_id: userId,
          nome: formDataServico.nome,
          codigo: formDataServico.codigo || null,
          descricao: formDataServico.descricao || null,
          valor_unitario: parseFloat(formDataServico.valor_unitario) || 0,
          categoria_id: formDataServico.categoria_id || null,
          unidade: formDataServico.unidade,
          observacoes: formDataServico.observacoes || null,
          ativo: true,
        })
  ...
  const handleAlterarStatus = async (id: string, novoStatus: 'pendente' | 'aprovado' | 'cancelado', isParcela: boolean = false) => {
    try {
      ...
      if (novoStatus === 'aprovado') {
        ...
        // Criar movimentação no fluxo de caixa apenas se não estava aprovado antes
        ...
        // Remover movimentação do fluxo de caixa se estava aprovado antes
        if (statusAnterior === 'aprovado') {
          await supabase
            .from('fluxo_caixa')
            .delete()
  ...
  const limparMovimentacoesOrfas = async () => {
    try {
      ...
      // Buscar todas as movimentações de vendas
  ...
  return (
    <MainLayoutEmpresarial>
      <div className="space-y-6">
        ...
        {/* Botão para criar produto/serviço */}
        <div className="flex justify-end">
          ...
          >
            ...
            <span>Cadastrar {abaAtiva === 'servico' ? 'Novo Serviço' : 'Novo Produto'}</span>
          </button>
        </div>
        ...
        {showModalServico && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[70] p-4">
            <div className="bg-gray-800 rounded-lg p-6 w-full max-w-2xl border border-gray-700 max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-white">Cadastrar Novo Serviço</h2>
                <button
              ...
              <form onSubmit={handleCriarServico} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={formDataServico.codigo}
                      onChange={(e) => setFormDataServico({ ...formDataServico, codigo: e.target.value })}
                      className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={formDataServico.unidade}
                      onChange={(e) => setFormDataServico({ ...formDataServico, unidade: e.target.value })}
                      className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                      placeholder="un, h (hora), d (dia)"
                    />
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Observações</label>
                  <textarea
                    value={formDataServico.observacoes}
                    onChange={(e) => setFormDataServico({ ...formDataServico, observacoes: e.target.value })}
                    rows={2}
                    className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                    placeholder="Observações adicionais..."
                  />
