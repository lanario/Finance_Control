...
export default function CartoesPage() {
  ...
  const toggleCartao = (cartaoId: string) => {
    if (cartaoExpanded === cartaoId) {
      setCartaoExpanded(null)
    } else {
      setCartaoExpanded(cartaoId)
    }
  }
              ...
              return (
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    {faturas.length > 0 && (
                      <button
                        onClick={() => toggleCartao(cartao.id)}
                            ...
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-400">
                                  Fecha: {new Date(fatura.dataFechamento).toLocaleDateString('pt-BR')} • 
                                  Vence: {new Date(fatura.dataVencimento).toLocaleDateString('pt-BR')}
                                </p>
                                {fatura.paga && fatura.dataPagamento && (
                                  <p className="text-xs text-green-400 mt-1">
                                    Paga em: {new Date(fatura.dataPagamento).toLocaleDateString('pt-BR')}
                                  </p>
                                )}
                              </div>
                                ...
                                {fatura.compras.map((compra) => (
                                  ...
                                  >
                                    <div className="flex-1">
                                      ...
                                      <p className="text-xs text-gray-400">
                                        {new Date(compra.data).toLocaleDateString('pt-BR')} • {compra.categoria}
                                      </p>
                                    </div>
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <p className={`text-xs mt-1 ${parcela.paga ? 'text-gray-500' : 'text-gray-400'}`}>
                                      ...
                                      {parcela.paga && parcela.data_pagamento && (
                                        <span className="ml-2">• Paga em: {new Date(parcela.data_pagamento).toLocaleDateString('pt-BR')}</span>
                                      )}
                                    </p>
                                  </div>
                                  <div className="flex items-center space-x-2 ml-4">
